...
PROTOTYPE_PATH = "/Users/vrindasharma/Desktop/project/CrediSphere-AI/prototype.html"
DEST_PATH = "/Users/vrindasharma/Desktop/CREDISPEHERAI-PROJECT/prototype.html"

with open(PROTOTYPE_PATH, 'r', encoding='utf-8') as f:
    html = f.read()

# 1. Check if styles for Collections AI already added
if "/* ============ COLLECTIONS AI ENGINE & LIVE TELEPHONY STYLES ============" not in html:
    css_to_add = """/* ============ COLLECTIONS AI ENGINE & LIVE TELEPHONY STYLES ============ */
#collections-ai-view {
...
.coll-insights-panel.open { display: block; }
.coll-insights-title {
  display: flex; align-items: center; justify-content: space-between;
  font-size: 12px; font-weight: 800; color: var(--navy); margin-bottom: 12px;
...
.q-meter-wrap { margin-top: 10px; display: flex; flex-direction: column; gap: 7px; }
.q-meter-row { display: flex; align-items: center; justify-content: space-between; font-size: 11px; gap: 10px; }
.q-meter-name { width: 145px; font-weight: 600; color: var(--ink); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
.coll-action-btn:hover { transform: translateY(-2px); box-shadow: 0 6px 16px rgba(15,26,58,0.12); }
.coll-action-btn .btn-icon { font-size: 18px; margin-bottom: 4px; }
.coll-action-btn .btn-title { font-size: 12px; font-weight: 700; }
.coll-action-btn .btn-subtitle { font-size: 9.5px; opacity: 0.82; margin-top: 2px; }

.coll-action-btn.primary {
  background: linear-gradient(135deg, var(--navy), var(--blue)); color: #fff;
  box-shadow: 0 2px 8px rgba(47,95,255,0.25);
}
...
/* Modals & Backdrop */
.coll-modal-backdrop {
  position: fixed; inset: 0; background: rgba(14,26,58,0.68);
  backdrop-filter: blur(4px); z-index: 1000;
  display: none; align-items: center; justify-content: center;
  animation: fadeInColl 0.2s ease;
}
.coll-modal-backdrop.open { display: flex; }
.coll-modal-box {
  background: #fff; border-radius: 16px; width: 100%; max-width: 520px;
  box-shadow: 0 24px 64px rgba(0,0,0,0.28); border: 1px solid rgba(255,255,255,0.4);
...
.call-borrower-name { font-size: 17px; font-weight: 800; margin-bottom: 2px; }
.call-borrower-num { font-size: 12px; color: rgba(255,255,255,0.7); font-family: 'DM Mono', monospace; }
.call-status-pill {
  display: inline-flex; align-items: center; gap: 6px;
...
.call-status-pill.connected { background: rgba(23,163,74,0.25); color: #4ade80; border: 1px solid rgba(74,222,128,0.3); }

.call-body { padding: 18px 22px; max-height: 230px; overflow-y: auto; background: var(--surface-2); }
.transcript-bubble {
  margin-bottom: 10px; display: flex; flex-direction: column; animation: fadeInColl 0.3s ease;
}
.transcript-bubble.agent { align-items: flex-end; }
.transcript-bubble.borrower { align-items: flex-start; }
.bubble-tag { font-size: 10px; font-weight: 700; color: var(--muted); margin-bottom: 3px; }
.bubble-content {
  max-width: 82%; padding: 9px 13px; border-radius: 12px; font-size: 12.5px; line-height: 1.45;
}
...
.template-select-wrap select {
  width: 100%; padding: 8px 10px; border-radius: 8px; border: 1px solid var(--border); font-size: 12px; font-family: inherit; margin-bottom: 8px;
}
.template-textarea {
  width: 100%; height: 95px; border-radius: 10px; border: 1px solid var(--border);
  padding: 10px 12px; font-family: inherit; font-size: 12px; line-height: 1.45; resize: none;
}
.msg-preview-card {
...
.ai-drawer-backdrop {
  position: fixed; inset: 0; background: rgba(14,26,58,0.5);
...
.ai-drawer-box {
  ...
  box-shadow: -12px 0 40px rgba(0,0,0,0.2); display: flex; flex-direction: column;
...
.ai-chip-prompt {
  ...
  padding: 8px 12px; font-size: 12px; font-weight: 600; text-align: left; cursor: pointer;
...
.ai-msg.bot .msg-bubble { background: #f8fafc; border: 1px solid var(--border); border-radius: 12px 12px 12px 2px; font-size: 12px; padding: 12px 15px; line-height: 1.55; color: #1e293b; }
...
.ai-drawer-send {
  background: var(--blue); color: #fff; border: none; border-radius: 10px; padding: 0 16px; font-weight: 700; cursor: pointer;
}
...
.coll-toast {
  ...
  box-shadow: 0 12px 32px rgba(0,0,0,0.22); border: 1px solid rgba(255,255,255,0.15);
...
"""
    # Insert right after `/* ============ COLLECTIONS AI (admin) ============ */`
    target_needle = "/* ============ COLLECTIONS AI (admin) ============ */"
    html = html.replace(target_needle, target_needle + "\n" + css_to_add)

# 2. Update the navbar button to have data-target="collections-ai"
html = html.replace(
    '<button class="pb-tab" data-frame="collections" onclick="showFrame(\'collections\',this)">Collections AI</button>',
    '<button class="pb-tab nav-link" data-frame="collections" data-target="collections-ai" onclick="showFrame(\'collections\',this)">Collections AI</button>'
)

# 3. Update sidebar items for Collections AI
sidebar_old = '<div class="sb-item active"><span class="ic">📞</span> Collections AI</div>'
sidebar_new = '<div class="sb-item active sidebar-menu-item" data-view="collections-ai" onclick="showFrameById(\'collections\')"><span class="ic">📞</span> Collections AI</div>'
html = html.replace(sidebar_old, sidebar_new)

sidebar_click_old = '<div class="sb-item" onclick="showFrameById(\'collections\')"><span class="ic">📞</span> Collections AI</div>'
sidebar_click_new = '<div class="sb-item sidebar-menu-item" data-view="collections-ai" onclick="showFrameById(\'collections\')"><span class="ic">📞</span> Collections AI</div>'
html = html.replace(sidebar_click_old, sidebar_click_new)

# 4. Replace the frame-collections content with the rich interactive module
old_frame_pattern = r'<div class="frame" id="frame-collections">.*?<footer class="proto-foot">CrediSphere AI · Collections Intelligence Module · MCA Project Prototype</footer>\s*</div>\s*</div>\s*</div>'

new_frame_html = """<!-- ================= COLLECTIONS AI ================= -->
<div class="frame" id="frame-collections" data-view="collections-ai">
  <div id="collections-ai-view">
    <div class="app-shell">
      <aside class="sidebar">
        ...
        <div class="sb-item"><span class="ic">📄</span> Loan Applications</div>
        ...
        <div class="sb-item"><span class="ic">⚙️</span> Risk Policy Config</div>
        ...
        <div class="content">
          ...
          <div class="grid-2" style="grid-template-columns:1fr 1.35fr 1fr;gap:18px;">
            ...
            <div class="card" id="coll-overdue-card">
              ...
              <div class="card-body">
                <div class="factor-row"><div class="factor-top"><b>Borrower Name</b><span>Farid Sheikh</span></div></div>
                <div class="factor-row"><div class="factor-top"><b>Contact Number</b><span style="font-family:'DM Mono',monospace;">+91 98765-43210</span></div></div>
                <div class="factor-row"><div class="factor-top"><b>EMI amount</b><span>₹1,20,000</span></div></div>
                <div class="factor-row"><div class="factor-top"><b>EMIs bounced</b><span>3 consecutive</span></div></div>
                <div class="factor-row"><div class="factor-top"><b>Total overdue</b><span style="color:var(--red);font-weight:700;font-size:14px;">₹3,72,000</span></div></div>
                <div class="factor-row"><div class="factor-top"><b>DPD bucket</b><span style="color:var(--red);font-weight:700;">90 (3)</span></div></div>
                
                <div class="coll-chip-wrap">
                  <span class="chip chip-rejected" id="coll-risk-badge">🔴 High risk (RF Risk 78.6%)</span>
                  <span class="coll-subchip warning">Broken Promises: 2</span>
                </div>
            ...
            <div class="card" id="coll-rec-card">
              <div class="card-head" style="display:flex;align-items:center;justify-content:space-between;">
                ...
                <button class="ai-engine-btn" id="coll-toggle-insights-btn" onclick="toggleMLEngineInsights()">
              ...
              <div class="card-body">
                ...
                <div class="coll-action-hub">
                  ...
                  <div class="coll-action-buttons">
                    ...
                    <button class="coll-action-btn outline" onclick="openSettlementModal()">
                      ...
                      <span class="btn-subtitle">₹40,000 Upfront Plan</span>
                    </button>
                  </div>
                </div>
                ...
                <div class="coll-insights-panel" id="coll-insights-panel">
                  <div class="coll-insights-title">
                    ...
                    <span style="font-size:10px;color:var(--muted);font-weight:600;">Updated Just Now</span>
                  </div>
...
html = re.sub(old_frame_pattern, new_frame_html, html, flags=re.DOTALL)

# 5. Add Modals & Drawers markup right before </body>
modals_markup = """
...
if "<!-- ================= COLLECTIONS AI MODALS & DRAWERS ================= -->" not in html:
    html = html.replace("</body>", modals_markup + "\n</body>")

# 6. Add the CollectionsMLEngine class and interactive handlers into the <script> block
script_code = """
/* ==========================================================================
...
class CollectionsMLEngine {
  constructor() {
    ...
    this.pipelineMetadata = {
      modelPipeline: "Random Forest (Risk Scoring) + DQN Agent (Dynamic Policy Optimization)",
...
document.addEventListener('DOMContentLoaded', () => {
  collectionsML.init();
  setupCollectionsNavTriggers();
});

// View Routing: Listen to navbar & sidebar triggers
function setupCollectionsNavTriggers() {
  document.querySelectorAll('.nav-link[data-target="collections-ai"], .sidebar-menu-item[data-view="collections-ai"]').forEach(el => {
    el.addEventListener('click', (e) => {
      e.preventDefault();
      showCollectionsAIView();
    });
  });
}

function showCollectionsAIView() {
  showFrameById('collections');
}

// Hook into showFrame to trigger Collections Engine
const originalShowFrame = window.showFrame;
window.showFrame = function(id, btn) {
...
function updateSMSPreview() {
  // Can live react to typing
}

function sendSMSLink() {
  const channelName = currentChannel === 'sms' ? 'SMS Gateway' : 'WhatsApp Business';
  const text = document.getElementById('sms-message-text').value;
  closeSMSModal();

  showCollToast(`${channelName} successfully dispatched via Gateway (Status: Delivered · Message ID: msg_${Math.random().toString(36).substring(7)})`, "success");
...
function askPresetQuestion(question) {
  ...
  setTimeout(() => {
    typingBubble.remove();
    const answer = AI_COPILOT_KNOWLEDGE[question] || "I have analyzed this parameter. Based on historical recovery data for DPD 90 accounts, the model prioritizes structured settlement over confrontational actions.";
...
# Insert script before </script>
html = html.replace("</script>", script_code + "\n</script>")

# Write back to both files
with open(PROTOTYPE_PATH, 'w', encoding='utf-8') as f:
    f.write(html)
print(f"Successfully updated {PROTOTYPE_PATH}")
